menu(type='*' where=(sel.count or wnd.is_taskbar or wnd.is_edit) title=title.terminal sep=sep.top image=icon.run_with_powershell)
{
	...
	item(title=title.Windows_Terminal tip=tip_run_admin admin=has_admin image='C:\ProgramData\Scoop\apps\windows-terminal\current\WindowsTerminal.exe' cmd="wt.exe" arg=`-d "@sel.path\."`)
}